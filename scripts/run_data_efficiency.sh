#!/usr/bin/env bash
# A3 data-efficiency curve: how much distilled data PER MATERIAL is enough?
# Sweep the number of rattled configs/material; same 16 materials (so element
# coverage is fixed — clean, unconfounded depth axis). Single-head FT.
# Headline metric: in-domain (train) MAE vs configs/material -> a clean
# "how much DFT data do you need" figure for the paper.
set -e
cd ~/phonon
...
EPOCHS="${1:-50}"
NCFGS="${2:-5 15 30 60}"   # random-rattle configs per material

for N in $NCFGS; do
  echo "=== DATA n_configs=$N ==="
  # regenerate dataset at this density (single-atom probes fixed at 3 sites)
  $PY scripts/make_finetune_data.py --out data/finetune --train $TRAIN --n-configs "$N" --n-single-sites 3
  NTR=$(grep -c Lattice data/finetune/train.xyz)
  ...
  echo "n_configs=$N -> $NTR train configs"
  OUT=results/ablation/ncfg$N; rm -rf "$OUT"; mkdir -p "$OUT"
  ...
  echo "=== EVAL n_configs=$N ==="
  $PY scripts/eval_finetune.py --baseline small --ft-model "$OUT/ft.model" --device cuda --ft-only \
    --train $TRAIN --holdout $HOLD --out "results/ablation/eval_ncfg$N.csv"
  echo "=== n_configs=$N DONE (ntrain=$NTR) ==="
done
echo "=== DATA-EFFICIENCY SWEEP COMPLETE ==="
