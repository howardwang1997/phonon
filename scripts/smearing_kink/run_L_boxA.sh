...
run_fc2(){ N=$1; SC=$2; W=results/v100/fc2_${N}_$(echo $SC|tr ',' '_')_0.015
  ...
  echo "[A-fc2] $N $(date +%T)"
  $PP scripts/v100/tmd_dft_fc2.py --name $N --config "$C" --pw /root/gpupw.sh --nproc 1 --pseudo-dir /root/phonon/pseudo --workdir "$W" 2>&1 | grep -E "min freq|FAIL" | tail -2; }
run_pp(){ N=$1; Y=$2; D=data/path_p_${N}
  [ -f "$D/train.xyz" ] && { echo "[A-pp] $N done"; return 0; }
  echo "[A-pp] $N $(date +%T)"
  $PP scripts/path_p_nbse2_make_data.py --yaml "$Y" --pw /root/gpupw.sh --pseudo-dir /root/phonon/pseudo --workdir results/path_p_${N} --outdir $D 2>&1 | grep -E "pathP|saved|Error|wrote" | tail -3; }
