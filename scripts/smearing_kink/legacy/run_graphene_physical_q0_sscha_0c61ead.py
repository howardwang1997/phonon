...
def phonopy_fc_to_cc_dyn(phonon, force_constants: np.ndarray):
    ...
    return tensor.GeneratePhonons(dimension), dimension, cc_for_phonopy
...
def operator_fc(freeze: dict, temperature: int) -> tuple[Path, np.ndarray]:
...
def initial_force_constants(
    ...
    _, diagonal_operator = operator_fc(freeze, lattice_temperature)
    _, target_operator = operator_fc(freeze, operator_temperature)
...
def main() -> int:
    ...
    dynamical, dimension, cc_for_phonopy = phonopy_fc_to_cc_dyn(phonon, initial_fc2)
    ...
    operator_path, operator = operator_fc(freeze, args.operator_temperature)
    reference = phonopy_to_ase(phonon.supercell)
    correction = FixedHarmonicCorrection(operator)
    calculator = FriedelMACECalculator(
        short_calculator, reference, correction, args.operator_temperature
    )
