...
TMUX= tmux new-session -d -s j050 "env DEGAUSS=0.05 NQ=3 NKF=24 WORK=/data/nbse2_d050 RESULT_CSV=/root/res_d050.csv bash $S/nbse2_epw_full.sh > /root/j050.log 2>&1"
TMUX= tmux new-session -d -s j060 "env DEGAUSS=0.06 NQ=3 NKF=24 WORK=/data/nbse2_d060 RESULT_CSV=/root/res_d060.csv bash $S/nbse2_epw_full.sh > /root/j060.log 2>&1"
...
TMUX= tmux new-session -d -s jconv "bash $S/graphene_lambda_conv.sh > /root/gr_conv.log 2>&1"
TMUX= tmux new-session -d -s jlq6 "env DEGAUSS=0.03 NQ=6 NKF=24 WORK=/data/nbse2_d030_q6 RESULT_CSV=/root/res_q6.csv bash $S/nbse2_epw_full.sh > /root/jlq6.log 2>&1"
