...
# All params via env vars so the driver can dispatch many of these in parallel:
#   JOB    unique name (-> results/ablation/eval_$JOB.csv, dir results/ablation/$JOB)
#   SEED   random seed (default 1)
#   NCFG   distilled rattle configs per material (default 30)
#   MODE   single | pt<N> | lora<R>     (anti-forgetting method)
#   MODEL  small | medium               (foundation model, default small)
#   TRAIN  space-separated mp-ids to fine-tune on
#   HOLD   space-separated held-out mp-ids (transfer test)
#   EPOCHS default 50
# Each job uses its OWN data dir so 8 can run concurrently without clobbering.
set -uo pipefail
cd ~/phonon
...
case "$MODE" in
  single) MH="--multiheads_finetuning False" ;;
  pt0)    MH="--multiheads_finetuning False" ;;
