#!/usr/bin/env bash
# Development-only refinement of the MACE batch-size speed/quality boundary.
set -euo pipefail
...
OUT="${OUT:-/data/graphene_physics_temperature/benchmarks/mace_convergence_refine_30ep_v100b}"
LOG="$OUT/run.log"

cd "$ROOT"
mkdir -p "$OUT"
exec 9>"$OUT/.refinement.lock"
if ! flock -n 9; then
    echo "MACE convergence refinement is already running"
    exit 0
fi
exec > >(tee -a "$LOG") 2>&1
if [[ -e "$OUT/DONE" || -e "$OUT/FAILED" ]]; then
    echo "MACE convergence refinement already has a terminal marker"
    exit 0
fi
...
for path in "$DATA/manifest.json" "$DATA/train.xyz" "$DATA/val.xyz" "$DATA/test.xyz" "$CONTROL"; do
    [[ -s "$path" ]]
    case "$path" in
        *T450*|*P4*|*p4*)
...
"$CONDA" run -n phonon python -c \
    'import json,sys; d=json.load(open(sys.argv[1])); assert d["status"]=="complete"; assert d["target_leakage"]["p4_450K_inputs_read"] is False; assert any(c["tag"]=="batch1_lr0p001" and c["status"]=="complete" for c in d["candidates"])' \
    "$CONTROL"
available_kb="$(df -Pk "$OUT" | awk 'NR==2 {print $4}')"
if [[ -z "$available_kb" || "$available_kb" -lt 52428800 ]]; then
    echo "refusing convergence refinement: less than 50 GiB free" >&2
    exit 75
fi
...
run_candidate() {
    ...
    local monitor_pid=$!
    local start_iso start_epoch end_iso end_epoch train_rc eval_rc
    start_iso="$(date -Is)"
    start_epoch="$(date +%s)"
...
overall_rc=0
run_candidate batch2_lrsqrt 2 0.0014142135623730952 || overall_rc=1
run_candidate batch4_lrsqrt 4 0.002 || overall_rc=1

git_head="$(git rev-parse HEAD 2>/dev/null || printf unknown)"
"$CONDA" run --no-capture-output -n phonon python \
    scripts/smearing_kink/summarize_graphene_mace_convergence_refinement.py \
    --root "$OUT" --control-summary "$CONTROL" \
    --data-manifest "$DATA/manifest.json" --output "$OUT/refinement_summary.json" \
    --git-head "$git_head"
summary_status="$($CONDA run -n phonon python -c \
    'import json,sys; print(json.load(open(sys.argv[1]))["status"])' \
    "$OUT/refinement_summary.json" | tail -n 1)"
...
echo "=== MACE convergence refinement COMPLETE $(date -Is) ==="
