...
support configurations, and per-atom ``node_feats``.  Every *linear-probe*
...
FEATURE_MODES = (FEATURE_MODE_SIGNED_L0, FEATURE_MODE_TENSOR_POWER)
...
class Probe:
    ...
    def predict(self, values: np.ndarray) -> np.ndarray:
        return np.asarray(self.model.predict(self.scaler.transform(values)), float)
...
def invariant_feature_schema(
    ...
    ``signed_l0`` is the historical R2K representation.  The optional tensor
    power mode preserves those signed scalar columns and appends one
    ...
    artificial angular-dimension scale factor.  No geometry, pooling, or
    cross-channel products enter either representation.
    """
    ...
    if feature_mode == FEATURE_MODE_TENSOR_POWER:
...
def replay_router_score_torch(
    ...
    """Replay StandardScaler plus scalar Ridge while retaining position gradients."""
    features = torch_invariant_features(node_feats, schema)
    mean = features.new_tensor(router_state["scaler_mean"])
    scale = features.new_tensor(router_state["scaler_scale"])
    coefficient = features.new_tensor(router_state["ridge_coefficient"])
    expected = int(schema["output_dimension"])
    if mean.numel() != expected or scale.numel() != expected:
        raise ValueError("router scaler dimension disagrees with feature schema")
    if coefficient.ndim != 1 or coefficient.numel() != expected:
        raise ValueError("router Ridge dimension disagrees with feature schema")
    ...
    scaler_relative_to_rms = scale / retained_rms
    ...
    return torch.sum((features - mean) / scale * coefficient, dim=1) + intercept
...
def fit_router(
    records: list[Record], family: str, alpha: float, utility_deadband: float
) -> Probe:
    ...
    return fit_ridge(values_array, target_array, weight_array, alpha)
...
def audit_family(
    ...
    for held_index, held in enumerate(support):
        ...
        router = fit_router(
            router_train + training_support, family, router_alpha, utility_deadband
        )
    ...
    full_router = fit_router(
        router_train + support, family, router_alpha, utility_deadband
    )
...
def supercell_score_invariance_audit(
    ...
    for size in (6, 8):
        ...
        score = float(
            np.dot((invariant - mean) / scale, coefficient)
            + float(router_state["ridge_intercept"])
        )
...
def main() -> int:
    ...
    fixed_geometry_overlap = {
        ...
        "interpretation": (
            ...
            "development; seed2 is only held out from the present linear-probe fit"
        ),
    }
    ...
    retained_dimensions = {
        value
        for name, value in router_state_dimensions.items()
        if name != "unpruned_feature_schema"
    }
    if len(retained_dimensions) != 1:
        raise RuntimeError(
            "router/scaler feature dimensions disagree: "
    ...
    if not all(
        ...
        for values in (router_scale, router_mean, router_coefficient)
    ...
    router_scaler_relative_to_training_rms = router_scale / retained_training_rms
    if np.any(
