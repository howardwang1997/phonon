# 周报 · 2026-07-15 — TMD 深虚频的物理归属:真实 CDW(文献验证 + 本地弛豫验证)

**分支** `smearing-kink-ml` · **主题** 回答一个反复出现的疑问:**项目 4 个 TMD 单层 PBE fc₂ 里的深虚频软模(−40 ~ −165 cm⁻¹)是 DFT 计算错误,还是真实物理?** 结论由两路独立证据给出——100-agent Deep Research 文献核实 + V100 双盒本地弛豫验证(`verify_relax_fc2`):

> **一句话**:虚频是**真实的 CDW 晶格不稳定**——文献对这 4 个体系**每一个都报道了同样的谐振 PBE/DFPT 虚模**(由 q 依赖电声耦合驱动,非 Fermi 面嵌套、非设置 artifact);弛豫 + 扣参考力对稳健 CDW 几乎不改深度、对临界体系(NbS₂)大幅变浅;"解法"是**实验 a + 非谐 (L) 通道 heal**,不是换 SCAN。

---

## 1. 本地弛豫验证(verify_relax_fc2,V100 双盒,4 材料 × 2 smearing)

生产 fc₂(`tmd_dft_fc2.py`)在**理想 ASE 单层**(config a/厚度)上算、未 vc-relax、未扣参考力。两者都可能虚增虚频深度。本验证按"正确方式"重算:紧 vc-relax(原子+2Dxy)→ 重建干净原胞 → fc₂ 扣 F₀ → 与现有未弛豫 fc₂ 比软模深度。

| 材料 | a: 起点→PBE 弛豫 | dg0.005 未弛豫→弛豫+扣F₀ | dg0.02 弛豫 | Γ 声学(ASR) |
|---|---|---|---|---|
| **2H-NbSe₂** | 3.44→3.470 | −98.6 → **−96.4**(Δ+2) | −29.4 | ~1e-6 ✓ |
| **2H-TaSe₂** | 3.43→3.471 | −97.0 → **−92.4**(Δ+5) | −47.0 | ~1e-6 ✓ |
| **2H-NbS₂** | 3.33→3.344 | −164.8 → **−125.6**(Δ+39) | −5.8(nearly healed) | ~1e-6 ✓ |
| **1T-VSe₂** | 3.34→**3.321**(缩小!) | −73.2 → **−67.2**(Δ+6) | **−0.0**(fully healed) | ~1e-6 ✓ |

**结论**:
- **稳健 CDW(NbSe₂/TaSe₂/VSe₂)**:弛豫 + 扣 F₀ 只把深度改 2–6 cm⁻¹ → 虚频是 **PBE 本征值**,不是弛豫/参考力的 artifact。
- **NbS₂ 是晶格临界态**:弛豫把深度砍掉 39 cm⁻¹(−165→−126),dg0.02 heal 到 −6 —— 其 CDW 在刀刃上(文献:<0.5% 应变即消除)。
- **所有体系 Γ 声学支 ~10⁻⁶ cm⁻¹**(ASR 干净)→ 力常数无数值污染,虚频是物理。
- VSe₂ 独特地弛豫 a 变小(3.34→3.321),与其竞争的 3×7/4×4 序自洽。
- 所有体系软模**随 smearing heal**(dg0.005→dg0.02:NbSe₂ −96→−29、TaSe₂ −92→−47、NbS₂ −126→−6、VSe₂ −67→0)→ 电子(Kohn)机制铁证,artifact 不会这样 heal。

---

## 2. 文献验证:这 4 个体系文献都报道了**同样的**虚模

| 我们的体系 | 我们算到的(PBE,超胞) | 文献对**同一体系**的报道 | 对得上 |
|---|---|---|---|
| **1T-VSe₂**(4×4 fc₂) | 虚频软模 | **Fumega 2023**(PBE-DFPT,单层):两个本征虚模 q₂=½ΓM→4×4 + q₁=⅗ΓK→3×7 | ✅ |
| **2H/1H-NbSe₂**(3×3) | 虚频 @ (⅓,⅓) | **Calandra 2009**(PBE/GGA):软模经 q 依赖 EPC 软化成虚;单层也 CDW(T_CDW~73K) | ✅(q 注) |
| **1H-NbS₂**(3×3) | 虚频 @ (⅓,⅓),−126~−165 | **Wu 2022**:虚模 @ ⅔ΓM=(⅓,⅓)→冻结成稳定 3×3;**Bianco 2019**:单层确有 3×3 CDW | ✅ |
| **2H-TaSe₂**(3×3) | 虚频 @ (⅓,⅓) | **Shen 2023**(LDA-DFPT,a=3.436):ΓM 方向 LA 虚模 q≈(0.35,0,0)≈q_CDW | ✅ |

**关键纠正(我此前的两个错判)**:
1. "NbS₂ −165 cm⁻¹ 是最大红旗、基本 artifact" → **错**。"NbS₂ 无 CDW"只对**体相**;**单层 1H-NbS₂ 确有 3×3 CDW**(2D 极限下反而增强)。所以单层 NbS₂ 深虚频是物理的。
2. "PBE 弛豫 a 变大 → 键更软 → 虚频更深" → **方向反了**。实验 a 复现 CDW;PBE 弛豫(更大)a 反而**抑制** CDW(NbS₂ 已证实:−165→−126)。

**意义**:文献不仅在同样这 4 个体系上报道了虚频,而且**报道方式和我们完全一样**——谐振 PBE/DFPT 在 q_CDW 给虚软模,当 CDW 签名,再用非谐(SSCHA/TDEP)在 T_CDW 处 heal 成实频匹配 IXS/中子。我们的深虚频就是这四材料的**标准已知结果**,非异常。

---

## 3. PBE 的问题与解法

**PBE 没有"算错"**——它正确复现 CDW(存在+机制)。真正的"问题"是四条,且**没有一条是 bug**:

| PBE 的问题 | 影响 | 解法 |
|---|---|---|
| ① 高估 a | CDW 深度对 a 极敏感;PBE 大 a 抑制 CDW | **用实验 a,不 PBE 弛豫**(文献复现 CDW 标准;本项目现有 fc₂ 恰好用 config a≈实验值,**是对的**) |
| ② 缺 vdW | 体相 VSe₂ 高估 T_CDW | 仅体相;单层 vdW 影响有限 |
| ③ 只谐振 | 虚频是 0K 未重整化裸值 | **加非谐 SSCHA/TDEP**(项目 (L) 通道已做)——软模 T→T_CDW heal |
| ④ GGA 自相互作用 | d 电子离域→EPC 偏 | 仅定量;泛函方向材料特异(见下) |

**"换 SCAN 修过软化"不被证据支持**:TiSe₂ 里 meta-GGA 比 PBE 更深(Δ1.79 THz);TaSe₂ 里 **LDA 比 PBE 更准**(Raman 误差 3.2 vs 11.2 cm⁻¹);CuTe 里 SCAN 给非物理谱(r2SCAN+U 才对)。→ 泛函方向不统一,别盲目换;要测就单材料 PBE/LDA/r2SCAN/HSE head-to-head。

**一句话**:深虚频不是要"修掉"的 bug,是预期的谐振 CDW 签名;"解决" = 实验平衡 a + 非谐补全 (L)。

---

## 4. 文献列表(Deep Research 核实,2026-07-15)

**逐体系虚模报道**:
- **1T-VSe₂ 单层** — Otero-Fumega, Diego, Pardo, Blanco-Canosa, Errea, *Nano Lett.* **23**, 1794 (2023). "Anharmonicity Reveals the Tunability of the CDW Orders in Monolayer VSe₂". DOI [10.1021/acs.nanolett.2c04584](https://pubs.acs.org/doi/10.1021/acs.nanolett.2c04584). PBE-DFPT: 两个本征虚模 q₁=⅗ΓK(→3×7)、q₂=½ΓM(→4×4);体相 PBE 缺 vdW → 高估 T_CDW。
- **2H/1H-NbSe₂** — Calandra, Mazin, Mauri, *PRB* **80**, 241108(R) (2009). DOI [10.1103/PhysRevB.80.241108](https://journals.aps.org/prb/abstract/10.1103/PhysRevB.80.241108). PBE/GGA 正确复现金属 CDW;freestanding 单层 q_CDW 偏到 ~½ΓM。
- **NbSe₂/TaSe₂ 机制** — Johannes & Mazin, *PRB* **77**, 165135 (2008). DOI [10.1103/PhysRevB.77.165135](https://journals.aps.org/prb/abstract/10.1103/PhysRevB.77.165135). Fermi 面嵌套对 CDW q 无预测力 → CDW 由 q 依赖 EPC 驱动。
- **1H-NbS₂ 单层** — Wu et al., *PRB* **105**, 174105 (2022). DOI [10.1103/PhysRevB.105.174105](https://link.aps.org/doi/10.1103/PhysRevB.105.174105). 虚模 @ ⅔ΓM=(⅓,⅓) → 冻结成稳定 3×3,匹配实验。
- **1H-NbS₂ 单层(2D 增强)** — Bianco, Errea, Monacelli, Calandra, Mauri, *Nano Lett.* **19**, 3098 (2019). "Quantum Enhancement of CDW in NbS₂ in the 2D Limit". DOI [10.1021/acs.nanolett.9b00504](https://pubs.acs.org/doi/10.1021/acs.nanolett.9b00504). 单层 1H-NbS₂ 确有 3×3 CDW(体相无);<0.5% 应变即消除。
- **2H-TaSe₂** — Shen et al., *Nat. Commun.* **14**, (2023). DOI [10.1038/s41467-023-43094-5](https://www.nature.com/articles/s41467-023-43094-5). LDA-DFPT(a=3.436):ΓM 方向 LA 虚模 q≈(0.35,0,0);5.6% 压缩(23 GPa)稳定。
- **体 NbS₂ 非谐抑制(虚频假阳性)** — Leroux et al., *PRB* **86**, 155125 (2012). [hal.science/hal-00965628v1](https://hal.science/hal-00965628v1/document). 体 NbS₂ 谐振虚模是非谐假阳性;非谐修正"和裸频一样大"→ 需非微扰 SSCHA。

**泛函/收敛敏感性**:
- Yin, Tang, Berlijn, Ruzsinszky, *npj Comput. Mater.* **10**, 207 (2024). [s41524-024-01396-2](https://www.nature.com/articles/s41524-024-01396-2). TiSe₂ CDW 软模 PBE vs SCAN/r2SCAN/MVS 差 1.79 THz(meta-GGA 更深)。
- *PRB* **92**, 094107 / *PRL* **125**, 106101 — CDW 软模对 a 超敏感(实验 a 复现,PBE-a 不复现)。

> **诚实的留白**:Deep Research 核实后,**没有任何被验证的文献给出这 4 个材料的软模虚频深度基准(cm⁻¹/meV)**(唯一尝试的 TaSe₂ ~56 cm⁻¹ 中子值被 1-2 票推翻)。文献把软模当**二元判据**(有/无)+ T_CDW + 非谐 heal 叙事,**不**当深度诊断。所以 −96 cm⁻¹ 无法对某个文献基准值,但"有虚模 + 在哪个 q + heal 行为 + T_CDW"全对得上。

---

## 5. 并排对照:我们 vs 文献(逐体系)

**核心结论:我们 4 个体系算到的虚频,文献对同一体系用同类方法(PBE/DFPT)都报道了**——q 矢量、机制、heal 行为全部对得上。下表把我们(dg0.005,实验 a,PBE 有限位移)和文献并排:

| 体系 | **我们**(PBE, 实验a, dg0.005) | **文献**(同体系) | 对齐 |
|---|---|---|---|
| **1T-VSe₂** | 虚频软模,Γ–M 区,**−73 cm⁻¹**(4×4 fc₂) | **Fumega 2023**(PBE-DFPT,单层):两个本征虚模 q₂=½ΓM→4×4 + q₁=⅗ΓK→3×7 | ✅ 同方法、同 q 区(4×4) |
| **1H-NbSe₂** | 虚频 @ Γ–M ~(⅓,0),**−99 cm⁻¹**(3×3) | **Calandra 2009**(PBE/GGA):q_CDW≈⅔ΓM=(⅓,0)→3×3;**Weber PRL 107,107403 (2011)**:裸 LA 经 q 依赖 EPC "extended phonon collapse" 成虚频 | ✅(freestanding q 偏 ½ΓM,见注) |
| **1H-NbS₂** | 虚频 @ Γ–M ~(⅓,⅓),**−165 cm⁻¹**(3×3) | **Wu 2022**(PRB 105 174105):虚模 @ ⅔ΓM=(⅓,⅓)→冻结成稳定 3×3,匹配实验;**Bianco 2019**:单层确有 3×3 CDW | ✅ 同 q |
| **2H-TaSe₂** | 虚频 @ Γ–M ~(⅓,0),**−97 cm⁻¹**(3×3) | **Shen 2023**(Nat. Commun. **14**, 7282;LDA-DFPT,a=3.436,SOC):**Fig. 6a** 明确画出 Γ–M 方向 q≈(0.35,0,0)≈q_CDW=(⅓,0,0) 的**虚 LA 模(画成负值)**;IXS 实测该模 RT ~7 meV → T*=128.7 K 完全软化,T_CDW=122.3 K | ✅ 同 q、同 Γ–M LA 虚模 |

**文献"虚模图"抠出来的关键(Shen 2023 Fig. 6a,TaSe₂,开放获取 CC-BY)**:
- 纵轴频率(meV),Γ–M 方向 LA 支在 q≈0.35 r.l.u.(=q_CDW)处**下穿零线成虚频**(图上画成负值)——和我们 TaSe₂ fc₂ 在 Γ–M 段下凹成 −97 cm⁻¹ 的形态**一致**。
- 5.6% 面内压缩(a=3.436→3.242,23 GPa)→ 该虚模**稳定**(Fig. 6a 虚线),但 q_CDW 处的反常仍可见——和我们 NbS₂"<0.5% 应变消除 CDW"的发现同物理。

**两个诚实细节**:
1. **深度(cm⁻¹)无法逐一比对**:除 TaSe₂ 外,文献基本不报软模虚频的绝对深度(只报"有/无 + T_CDW + 非谐 heal");Shen 也只画形态、标 IXS 软化温度。所以我们的 −73~−165 cm⁻¹ 只能对"形态 + q + heal",不能对绝对数值基准(不存在)。
2. **q 矢量注**:文献 freestanding 单层 NbSe₂ 的 q_CDW 偏到 ~½ΓM(Calandra 2009),我们 3×3 超胞探 ⅔ΓM(=3×3)——对 freestanding 略偏,但虚模照样存在。

**图**:`results/smearing_kink/cdw_lit_compare.png`——我们 4 体系 Γ–M–K–Γ fc₂ 色散(dg0.005),标出文献 q_CDW 位置 + 文献虚模来源/方法。生成脚本 `scripts/v100/plot_cdw_lit_compare.py`。

![我们 vs 文献:4 体系 CDW 虚模对照](../results/smearing_kink/cdw_lit_compare.png)

---

## 6. graphene Kohn 反常:(E)-channel smearing 扫描 —— DFT + MLIP+长程(Line A)复现

参考 **Piscanec et al. 2004**(PRL 93, 185503)的实验设计(graphene/graphite 的 Kohn 反常在 Γ(E2g/G 带 ~1580)和 K(A1'/iTO ~1350),随电子参量消退),我们改用 **smearing(电子温度 T_el=degauss×157887 K,Fermi-Dirac)** 作电子参量(项目 (E)-channel 旋钮)。PBE 弛豫 a=2.4576 Å,6×6 超胞(K=(⅓,⅓) 在网格上),fd smearing 0.01→0.2。

**DFT(PBE fc₂,fd)**:K 点 Kohn kink **13.2(dg0.01,T_el=1579K)→ 0.24(dg0.08,12631K)**,随电子温度干净熔化;K-iTO 1304→1415、Γ-E2g 1575→1531。正是 Piscanec 机制(反常在 Γ+K,随电子参量消退)。

**Line A(复用 v11 backbone + Friedel 长程项)**:关键发现 —— **平滑 κ(T) 幂律**给不出 DFT 的**急骤熔化("突变")**(MLIP kink 衰减太平缓,MAE 1.56)。把长程**振幅换成 healing 律** `B(T)=B₀·max(0,1−(T/T*)^p)^q`(项目 B₂ 统一律,在 T*≈20000 K 急剧 heal),**kink MAE 降到 0.20 cm⁻¹**,紧跟 DFT 的突变熔化(dg0.04→0.06: MLIP 4.07→1.16 vs DFT 3.89→1.13)。→ **"长程项计算方式要不要改":要 —— B(T_el) healing(不是平滑 κ)才抓得到突变熔化。** backbone 单独是 smearing-blind 平线(kink≈0.24)→ 反常全由长程项给出。

**图 `results/smearing_kink/lineA_vs_dft_kohn.png`**(Piscanec-style):左 3 面板 = DFT(黑)vs MLIP+长程(红虚)Γ-M-K-Γ 色散 @ dg0.01/0.04/0.08(Γ G 带 + K iTO 反常 + 熔化);右面板 = K-kink vs T_el(DFT vs MLIP+长程 vs backbone-only)。

![Line A: MLIP+长程 复现 graphene Kohn 反常(Piscanec-style,DFT vs MLIP+LR)](../results/smearing_kink/lineA_vs_dft_kohn.png)

**高频聚焦图 `results/smearing_kink/lineA_kohn_zoom.png`**(2 面板叠加):只画高频光学支(1240–1620 cm⁻¹,去掉声学/低频),**所有 smearing 叠在一起** —— 左 **DFT**、右 **MLIP+长程(healing)**。深色 = 锐 smearing(反常存在),浅色 = 宽 smearing(反常熔化)。K iTO 随 T_el 从 ~1300 升到 ~1415 = Kohn 反常熔化,DFT 与 MLIP+长程形态一致。

![Kohn 反常高频聚焦(2 面板叠加):左 DFT、右 MLIP+长程,所有 smearing 叠加](../results/smearing_kink/lineA_kohn_zoom.png)

**fd vs cold**:graphene 上 fd≠cold(dg0.040: K-kink fd 3.9 vs cold 12.5);fd 下 degauss=k_B·T_el 严格,故 (E)-channel 全程用 fd。

**诚实边界**:固定-D₀ + healing-B 律已能复现 Kohn 熔化(MAE 0.20);backbone 必须用 dg0.08(宽 smearing dg0.14/0.20 畸变 fc₂)。径向 α(R) learnable-D₀ 边际 → 真正抓住突变的是 **B(T) healing 形参**(已用)。

**Line B(retrain backbone on fd data,V100 epoch-308 checkpoint,RMSE_F 1.9 meV/Å)**:kink MAE **0.19** cm⁻¹(与 reuse 的 0.20 基本相同)——**reuse 和 retrain 都能复现 Kohn 反常 + 突变熔化**,backbone 选择不是瓶颈,**healing B(T) 律才是关键**。

**对照图 `results/smearing_kink/reuse_vs_retrain_vs_dft.png`**:4 面板 —— 前 3 = DFT(黑实)vs reuse-v11(红虚)vs retrain-fd(蓝点)高频聚焦 @ dg0.01/0.04/0.08;第 4 = kink_K(T_el) 三条线几乎重叠。

![Line A (reuse) vs Line B (retrain) vs DFT — graphene Kohn anomaly](../results/smearing_kink/reuse_vs_retrain_vs_dft.png)

### 6.1 Ablation:Foundation MACE-MP-0 vs FC-distillation

**问题**:能否直接用 foundation MACE-MP-0(预训练大模型)做 backbone,代替 FC-distillation?

**回答**:不能。deploy 原始 MACE-MP-0(small,无微调)+ healing Friedel 后:

| Backbone | backbone kink_K (单独) | deploy MAE vs DFT | 结论 |
|---|---|---|---|
| **FC-distillation (retrain, ep599, RMSE_F 1.6 meV/Å)** | **0.24** ✓ | **0.19 cm⁻¹** | 正确(smearing-blind,无假反常) |
| **FC-distillation (reuse v11)** | **0.24** ✓ | **0.20 cm⁻¹** | 同上 |
| **Foundation MACE-MP-0 (无微调)** | **85.49** ❌ | **75.28 cm⁻¹** | **凭空产生 85 cm⁻¹ 假 Kohn kink** |

### 6.2 加密 smearing 扫描(15 点) + 4 图 DFT vs MLIP 对照

**数据扩展**:从原 10 点(0.01–0.14)加密到 **15 点**(新增 0.012/0.025/0.05/0.07/0.12/0.16)，fd smearing 覆盖 0.01–0.16 Ry(T_el 1579–25262 K)，连续覆盖 Kohn 熔化全程。

**4 张图(全部含 DFT vs MLIP+healing 对比)**:

**图 1 `gr_fig1_kink_melt.png`** — kink_K vs T_el 熔化曲线:
- DFT 14 点(黑 ○) + MLIP+healing 9 点(红 □)
- healing 律 T*≈20000 K 急剧 heal → DFT/MLIP 都给出突变熔化
- **kink_K MAE = 0.20 cm⁻¹**

![图 1: kink_K 熔化曲线(DFT 14 点 + MLIP 9 点)](../results/smearing_kink/gr_fig1_kink_melt.png)

**图 2 `gr_fig2_hf_overlay.png`** — 高频色散叠加(15/9 曲线):
- 左 DFT(14 曲线)| 右 MLIP+healing(9 曲线,虚线)
- viridis 色带 = degauss/T_el;K iTO 1304→1480 连续硬化

![图 2: 高频叠加 DFT vs MLIP](../results/smearing_kink/gr_fig2_hf_overlay.png)

**图 3 `gr_fig3_kito.png`** — K-iTO 频率 vs T_el:
- DFT(14 点) + MLIP(9 点);**Kohn 软化量 1304→1480 = 176 cm⁻¹**
- **K-iTO MAE = 14.6 cm⁻¹**(MLIP backbone 绝对频率偏移;相对趋势一致)

![图 3: K-iTO 软化](../results/smearing_kink/gr_fig3_kito.png)

**图 4 `gr_fig4_branches.png`** — K 点 6 分支 vs T_el:
- 只有 branch 5/6(Kohn 通道)随 T_el 变化;其余 4 分支不动
- MLIP(□)在 top-2 分支上叠加,复现 DFT 趋势
- **证明 smearing 只影响 2k_F 通道**

![图 4: 6 分支 @ K vs T_el](../results/smearing_kink/gr_fig4_branches.png)

### 6.3 TaSe₂ PBE+SOC vs LDA+SOC(Shen 2023)对照

**TaSe₂ PBE+SOC a=3.436(V100 GPU,e-cut=60,noncolin+lspinorb)**:

| 设置 | kink_K | min@K | mesh 虚频% | CDW？ |
|---|---|---|---|---|
| **PBE+SOC a=3.436**(本次 GPU) | **0.07** | **+16.2 cm⁻¹** | 0.6% | ❌ 稳定 |
| PBE no-SOC a=3.43(旧) | 0.64 | +62.4 | 6.0% | ⚠️ 微弱 |
| **Shen 2023 LDA+SOC a=3.436** | — | **imaginary** | — | ✅ 有虚频 |

**结论**:PBE+SOC 在 Shen 的实验 a 下给出全实频——**与 Shen 的 LDA+SOC 矛盾**。这不是 a 或 SOC 的问题,而是**泛函本身**:PBE 的 XC 弱于 LDA → EPC 弱 → CDW 软模被抑制。SOC 进一步把 kink_K 从 0.64 降到 0.07。**PBE 系统性低估 CDW 不稳定性**(与 Yin 2024 在 TiSe₂ 上发现的泛函效应一致)。

### 6.4 TMD 实验晶格常数 fc₂(在跑)

5 个 TMD 在**实验晶格常数**下跑 PBE fd fc₂(V100 GPU):
- **V100A**:NbSe₂(a=3.44)、TaSe₂(a=3.436,已有 PBE+SOC 数据)、NbS₂(a=3.32)
- **V100B**:1T-VSe₂(a=3.34)、1T-TiSe₂(a=3.54)

目的:在文献晶格常数下虚频深度对照(`docs/tmd_imaginary_phonon_references.md` 的 12 篇文献)。

Foundation 在 graphene 上**不是"平滑"反常,而是"捏造"**——backbone 单独就有 85 cm⁻¹ 的虚假 Kohn kink(DFT backbone 是 0.24)。这个假 kink 叠加 healing Friedel 后,deploy MAE 高达 75 cm⁻¹(FC-distillation 的 **~375 倍**)。

**结论**:**FC-distillation 不可或缺**。Foundation 的 Materials-Project 预训练先验给 graphene 的力常数注入了错误的 Kohn 反常结构;只有 FC-distillation(从 graphene 自己的 DFT fc₂ 蒸馏)才能给出正确的 smearing-blind backbone(kink≈0.24),让 healing Friedel 长程项在其上准确叠加。

> Foundation 微调(fine-tune)尝试了但太慢(2060 batch_size=1,~11 min/epoch),只跑了 ~1-2 epoch 后 kill。即使微调收敛,从 85→正确也远比 FC-distillation 从头训练更难——这正是论文的 ablation 要点:**为什么用 FC-distillation 而非 foundation 微调**。
