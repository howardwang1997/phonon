...
- DFT 15 点(黑 ○) + MLIP+healing 15 点(红 □)
- healing 律在 smearing*≈`0.127 Ry` 附近急剧 heal → DFT/MLIP 都给出突变熔化
- **kink_K MAE = 0.24 cm⁻¹**

![图 1: kink_K 随 smearing 熔化(DFT 15 点 + MLIP 15 点)](../results/smearing_kink/gr_fig1_kink_melt.png)

**图 2 `gr_fig2_hf_overlay.png`** — 高频色散叠加(15/15 曲线):
- 左 DFT(15 曲线) | 右 MLIP+healing(15 曲线，虚线)
...
- DFT(15 点) + MLIP(15 点)；**Kohn 软化量 1304→1480 = 176 cm⁻¹**
- **K-iTO MAE = 17.8 cm⁻¹**。模型在低到中等 smearing 区间跟随 DFT 硬化，但在 `0.10 Ry` 以上约饱和在 `1415 cm⁻¹`，因此高-smearing 绝对频率还未完全复现。
...
- MLIP 在 top-2 分支上用虚线方形 marker 叠加，复现 DFT 的主要趋势
