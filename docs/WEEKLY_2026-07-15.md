...
**Line A(复用 v11 backbone + 调参 Friedel 长程项)**:以 **kink MAE 1.56 cm⁻¹** 复现 Kohn 反常及其熔化。关键:backbone 单独是 **smearing-blind 的平线(kink≈0.24)** —— 反常**完全由长程 Friedel 项给出**,MLIP+长程紧跟 DFT。
...
**高频聚焦图 `results/smearing_kink/lineA_kohn_zoom.png`**:只画高频光学支(1240–1620 cm⁻¹,去掉声学/低频杂讯),聚焦 Kohn 反常点 —— **Γ 的 G 带(E2g ~1575)** 和 **K 的 iTO(A1' ~1300)**,DFT vs MLIP+长程 @ dg0.01/0.04/0.08。反常随 T_el 熔化清晰可见(K iTO 从 ~1300 升到 ~1415)。

![Kohn 反常高频聚焦:Γ G 带 + K iTO,DFT vs MLIP+长程,随 T_el 熔化](../results/smearing_kink/lineA_kohn_zoom.png)

**fd vs cold**:graphene 上 fd≠cold(dg0.040: K-kink fd 3.9 vs cold 12.5);fd 下 degauss=k_B·T_el 严格,故 (E)-channel 全程用 fd。

**诚实边界**:固定-D₀ Friedel 在 [ref, backbone=dg0.08] 内合理(MAE ~1),但宽 smearing(dg0.14/0.20)会整体畸变 fc₂ → 不能做 backbone(必须用 dg0.08)。径向可学习-D₀(α(R))仅边际改善 → 宽范围真正缺失的是 D₀(T_el)。Line B(在 fd 数据上 retrain backbone,V100 进行中)将进一步收紧。
