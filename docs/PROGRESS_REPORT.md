...
非谐(3 阶力常数 → 晶格热导 κ)是最主要的自产需求。成本 = phono3py 在超胞上跑
**N 次 SCF**;3 阶位移数 N 远大于谐性(~N² 量级)。基于 `glm` 分支**实测**的
QE-GPU 单 SCF(16 原子 20s、64 原子 94s;外推 128 原子 ~5–8 min)+ phono3py 典型
位移数:

| 材料类型 | N(位移) | 单 SCF | **每材料 GPU-hr**(单卡等效) |
|---|---|---|---|
| Si / 高对称简单 | ~110 | 94 s | **~3** |
| 岩盐/闪锌矿(64 原子超胞) | ~200–500 | 1.5–3 min | **~5–25** |
| 中等(128 原子,3–4 元) | ~800–1500 | 5–8 min | **~70–200** |
| 复杂/低对称 | 2000–5000+ | 5–10 min | **~200–800** |

位移彼此独立 → 8× H20 上墙钟 = GPU-hr / 8。数据集级:

| 场景 | 规模 | GPU-hours | 8×H20 墙钟 |
|---|---|---|---|
| 小验证集(10–20 简单-中等材料) | ~15 | **~300–800** | ~2–4 天 |
| 中等基准集(~50 材料) | 50 | ~1,500–4,000 | ~1–2 周 |
| 大集(~200 材料) | 200 | ~6,000–15,000+ | 数周 |

**关键策略(避免硬跑全 DFT)**:① 公开非谐数据先用(Togo phono3py ~100 化合物,**0
GPU-hr**);② 微调 MLIP 算大批非谐(力评估几乎免费,可覆盖上千材料);③ **只自产一个
小 DFT 验证集(~10–20 材料)校验 MLIP 非谐** → **~300–800 GPU-hr(≈ 8×H20 跑
2–4 天)**。
→ 一句话:严肃全 DFT 非谐是 **数千–上万 GPU-hr**;走"公开 + MLIP + 小验证"路线,
**自产部分只需 ~300–800 GPU-hr**就够支撑一篇文章的非谐验证。
